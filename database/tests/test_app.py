...
def test_create_sale(test_client, user_token):
...
def test_fetch_customer_reviews(test_client, user_token):
    response = test_client.get('/reviews/customer/admin', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)
...
def test_get_recommendations(test_client, user_token):
    response = test_client.get('/recommendations/admin', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)
