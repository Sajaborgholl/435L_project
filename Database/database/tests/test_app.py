...
def user_token():
    ...
    # Assuming 0 is the role for a normal user
    return generate_jwt("johndoe", 0)
...
def test_fetch_customers(test_client, admin_token):
    response = test_client.get(
        '/db/customers', headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 200
...
def test_fetch_customer(test_client, admin_token):
    response = test_client.get(
        '/db/customers/johndoe', headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 200
...
def test_charge_customer(test_client, admin_token):
    response = test_client.post('/db/customers/johndoe/charge', headers={
                                "Authorization": f"Bearer {admin_token}"}, json={"amount": 100000})
    assert response.status_code == 200


def test_subtract_customer(test_client, admin_token):
    response = test_client.post('/db/customers/johndoe/deduct', headers={
                                "Authorization": f"Bearer {admin_token}"}, json={"amount": 100})
    assert response.status_code == 200
...
def test_fetch_inventory(test_client, admin_token):
    response = test_client.get(
        '/db/inventory', headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)
...
def test_delete_inventory_item(test_client, admin_token):
    response = test_client.delete(
        '/db/inventory/1', headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 200
...
def test_create_sale(test_client, user_token, admin_token):
    ...
    test_client.post('/db/customers/johndoe/charge',
                     headers={"Authorization": f"Bearer {admin_token}"}, json={"amount": 100000})
...
def test_fetch_sales(test_client, admin_token):
    response = test_client.get(
        '/db/sales', headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)
...
def test_get_goods_sales(test_client, user_token):
    response = test_client.get(
        '/db/sales/goods', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)
...
def test_get_specific_goods_sales(test_client, user_token):
    response = test_client.get(
        '/db/sales/good/2', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200


def test_get_customer_sales(test_client, user_token):
    response = test_client.get(
        '/db/sales/customer/johndoe', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)


def test_add_to_wishlist(test_client, user_token):
    response = test_client.post('/db/sales/wishlist/johndoe', headers={
                                "Authorization": f"Bearer {user_token}"}, json={"product_id": 2})
    assert response.status_code == 200


def test_get_wishlist(test_client, user_token):
    response = test_client.get(
        '/db/sales/wishlist/johndoe', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)
...
def test_get_user_recommendations(test_client, user_token, admin_token):
    ...
    products = [
        {"name": "Product A", "category": "Electronics", "price": 100.00,
            "description": "Description A", "stock": 10},
        {"name": "Product B", "category": "Electronics", "price": 150.00,
            "description": "Description B", "stock": 10},
        {"name": "Product C", "category": "Electronics",
            "price": 200.00, "description": "Description C", "stock": 10}
    ]
    for product in products:
        test_client.post(
            '/db/inventory', headers={"Authorization": f"Bearer {admin_token}"}, json=product)
    ...
    # Get recommendations
    response = test_client.get('/db/sales/recommendations/johndoe',
...
def test_update_review(test_client, user_token):
    ...
    response = test_client.get(
        '/reviews/product/2', headers={"Authorization": f"Bearer {user_token}"})
    assert response.status_code == 200
...
def test_fetch_reviews(test_client, admin_token, user_token):
    ...
    response = test_client.get(
        f'/reviews/{review_id}', headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 200
